...
public class AnonymizeDriver extends Configured implements Tool
{
    ...
    public int run(String[] args) throws Exception
    {
        ...
        Job job = Job.getInstance(conf, "Anonymize Hadoop Job");
        job.setJarByClass(AnonymizeDriver.class);
        ((JobConf) job.getConfiguration()).setJar(jarFile.toString());
        job.setMapperClass(AnonymizeMapper.class);// 执行用户自定义map函数
        job.setReducerClass(AnonymizeReducer.class);// 执行用户自定义reduce函数
        job.setNumReduceTasks(10);
        //设置输出类型
        job.setOutputKeyClass(IntWritable.class);
        job.setOutputValueClass(Text.class);

        System.out.println("Anonymize Hadoop Job start!");
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        int exitCode = ToolRunner.run(new AnonymizeDriver(), otherArgs);
        if (exitCode == 0)
        {
