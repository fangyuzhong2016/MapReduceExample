 ...
 */
public class InvertedIndexDrive extends Configured implements Tool
{
    ...
    public int run(String[] args) throws Exception
    {
        ...
        Job job = Job.getInstance(conf, "InvertedIndex Hadoop Job");
        ...
        job.setMapperClass(InvertedIndexMapper.class);// 执行用户自定义map函数
        job.setCombinerClass(InvertedIndexCombiner.class);// 对用户自定义map函数的数据处理结果进行合并，可以减少带宽消耗
        job.setReducerClass(InvertedIndexReducer.class);// 执行用户自定义reduce函数

        //设置一下Map输出类型
        job.setMapOutputKeyClass(AddressKeyWord.class);
        ...
        System.out.println("InvertedIndex Hadoop Job start!");
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        int exitCode = ToolRunner.run(new InvertedIndexDrive(), otherArgs);
        if (exitCode==0)
        {
